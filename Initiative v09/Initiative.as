package  {
	...
	public class Initiative extends MovieClip{
		...
		private function createInitPrompt(e:*,index:int,array:Array)
		{
			...
			var text1:TextField = new TextField2(charStatFormat,e.myName + "'s initiative:",615,410 + index * 50,250);
			...
			var buttonLabel:TextField = new TextField2(inputFormat,"Done",button.x + 2,button.y + 5,70);
		...
		private function createMainPrompts()
		{
			...
			var moveButton:BasicButton = new BasicButton();
			moveButton.x = statusButton.x + statusButton.width + 5;;
			moveButton.y = 450;
			moveButton.width = 170;
			moveButton.addEventListener(MouseEvent.CLICK,chooseMoveCreature);
			buttons.push(moveButton);
			addChild(moveButton);
			
			//The label for the move button
			var moveLabel:TextField = new TextField();
			moveLabel.mouseEnabled = false;
			moveLabel.defaultTextFormat = inputFormat;
			moveLabel.text = "Move";
			moveLabel.x = moveButton.x + 40;
			moveLabel.y = moveButton.y + 5;
			moveLabel.height = moveLabel.textHeight + 5;
			buttonLabels.push(moveLabel);
			addChild(moveLabel);
		}
		...
		//Called by the move button. Moves a creature in the initiative list
		private function chooseMoveCreature(event:MouseEvent)
		{
			//Return if there are no creatures
			if(creatures.length < 1)
				return;
			...
			var text1:TextField = new TextField();
			text1.defaultTextFormat = charStatFormat;
			text1.text = "Select a creature to move";
			text1.x = 615;
			text1.y = 410;
			text1.width = 350;
			text1.mouseEnabled = false;
			...
			cancelButton.addEventListener(MouseEvent.CLICK,
				function cancelMove(e:MouseEvent)
				{
					createMainPrompts();
					initiativeOrder.forEach(function noMouseList(e:*,i:int,a:Array)
									{e.mouseEnabled = false; e.removeEventListener(MouseEvent.CLICK,chooseMoveLocation);});
				});
			buttons.push(cancelButton);
			addChild(cancelButton);
			...
			for(var i:int=0;i<initiativeOrder.length;i++)
			{
				initiativeOrder[i].addEventListener(MouseEvent.CLICK,chooseMoveLocation);
				initiativeOrder[i].mouseEnabled = true;
			}
		}
		
		//Called when a creature is selected to be moved in the initiative list
		private function chooseMoveLocation(event:MouseEvent)
		{
			//Get the number of the creature that was clicked
			var creatureNum:int = initiativeOrder.indexOf(event.target);
			...
			//Text that directs the user to select a destination
			var text1:TextField = new TextField();
			text1.defaultTextFormat = charStatFormat;
			text1.text = "Select the new position";
			text1.x = 615;
			text1.y = 410;
			text1.width = 350;
			text1.mouseEnabled = false;
			...
			cancelButton.addEventListener(MouseEvent.CLICK,
				function cancelMove(e:MouseEvent)
				{
					createMainPrompts();
					initiativeOrder.forEach(function noMouseList(e:*,i:int,a:Array)
									{e.mouseEnabled = false; e.removeEventListener(MouseEvent.CLICK,moveCreature);});
				});
			buttons.push(cancelButton);
			addChild(cancelButton);
			...
			for(var i:int=0;i<initiativeOrder.length;i++)
			{
				initiativeOrder[i].addEventListener(MouseEvent.CLICK,moveCreature);
				initiativeOrder[i].mouseEnabled = true;
			}
		}
		
		//Called when a creature and location of a move are selected
		private function moveCreature(event:MouseEvent)
		{
			//Get the number of the creature that was clicked
			var destinationNum:int = initiativeOrder.indexOf(event.target);
			
			if(selectedCreatureNum!=destinationNum)
			{
				serverConnection.send("MoveCreature",selectedCreatureNum,destinationNum);
				//Compensate for the moved creature
				if(activeCreatureNum > selectedCreatureNum && activeCreatureNum < destinationNum)	
					activeCreatureNum--;
				else if(activeCreatureNum <= selectedCreatureNum && activeCreatureNum >= destinationNum)
				{
					if(activeCreatureNum == creatures.length-1)
						activeCreatureNum = 0;
					else
						activeCreatureNum++;
				}
				
				//Move that creature from selectedCreatureNum to destinationNum
				var tempCreature:Creature = (Creature)(creatures.splice(selectedCreatureNum,1)[0]);
				if(selectedCreatureNum > destinationNum)
					creatures.splice(destinationNum,0,tempCreature);
				else
					creatures.splice(destinationNum-1,0,tempCreature);
			}
			//Remove the listeners
			for(var i:int=0;i<initiativeOrder.length;i++)
			{
				initiativeOrder[i].removeEventListener(MouseEvent.CLICK,moveCreature);
				initiativeOrder[i].mouseEnabled = false;
			}
			createMainPrompts();
			refreshInitiative(false);		//Refresh the initiative list without sorting
		...
		private function handleConnect(client:Client):void
		{
			...
			client.multiplayer.createJoinRoom(
				"DnD",								//Room id. If set to null a random roomid is used
		...
		private function handleJoin(connection:Connection):void
		{
			...
			var pcLoader:URLLoader = new URLLoader(new URLRequest("http://dungeonmarkster.site88.net/Initiatives/PlayerCharacters.xml"));
