package  {
	...
	public class Initiative extends MovieClip{
		...
		private function addCreature(event:MouseEvent)
		{
			...
			serverConnection.send("AddCreature",textFields[0].text,int(textFields[1].text));
			textFields[0].text = "";
			textFields[1].text = "";
		...
		private function editCharInit(event:MouseEvent)
		{
			...
			playerCharacters[buttonNum].initiative = int(textFields[buttonNum*2+1].text);
			serverConnection.send("EditCharInit",buttonNum,int(textFields[buttonNum*2+1].text));
		...
		private function handleMessages(m:Message){
			...
			{
				case "integer":
					trace("I just got sent the number " + m.getInt(0));
					break;
				case "Join":
